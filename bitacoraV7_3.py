...
CONFIG = {
    # ── Excel principal ────────────────────────────────────────────
    # Solo una VISTA exportada desde SQLite. V7 lo sobreescribe en
    # cada corrida. Nombre: el Excel que corregiste y usaste en bootstrap.
    "RUTA_EXCEL": (
        r"C:\Users\s4930359\OneDrive - The Bank of Nova Scotia"
        r"\Bitacora_Reglas\Bitacora_Master_Final.xlsx"
    ),
    # ── Excel pendientes ───────────────────────────────────────────
    # No necesitas crearlo — V7 lo genera automático si hay correos
    # con formato inválido. Si no hay pendientes, no se genera.
    "RUTA_EXCEL_PENDIENTES": (
        r"C:\Users\s4930359\OneDrive - The Bank of Nova Scotia"
        r"\Bitacora_Reglas\Bitacora_Pendientes_Formato.xlsx"
    ),
    # ── SQLite — FUENTE DE VERDAD ──────────────────────────────────
    # Mismo Respaldo_Blindado.db ya reseteado. Correlativo max: 3009.
    "RUTA_DB_SQLITE": (
        r"C:\Users\s4930359\OneDrive - The Bank of Nova Scotia"
        r"\Bitacora_Reglas\Respaldo_Blindado.db"
    ),
    # ── Backup .msg ────────────────────────────────────────────────
    # Igual que V6 — misma ruta, no se toca.
    "RUTA_BACKUP_MSG": (
        r"C:\Users\s4930359\OneDrive - The Bank of Nova Scotia"
    ...
    # ── Regex ID — versión robusta ─────────────────────────────────
    # Captura el ID entre [ ] excluyendo espacios y texto extra
    # que a veces agregan (ej: S/N, N/A al final del ID).
    # Ejemplos válidos:
    #   [ID-VRM-3010]       → ID-VRM-3010
    #   [ID_PMFD_2990]      → ID_PMFD_2990
    #   [ID-RT TD-001 S/N]  → ID-RT  (S/N queda fuera)
    #   [ID-FRM-001]        → ID-FRM-001
    # Captura TODO lo que esté dentro de [ ] sin restricciones.
    # Aguanta espacios, guiones, S/N, o cualquier variación de formato.
    "REGEX_ID": r"\[([^\]]+)\]",
}
...
class ProcesadorTexto:
    ...
    def extraer_id(asunto: str):
        """Captura todo lo que esté dentro de [ ] en el asunto."""
    ...
    if not sol_folder:
        ...
        for m in msgs:
            try:
                ...
                # No se ignora — se registra con correlativo propio
                # y campos vacíos para completar manualmente en Excel.
                if sql.existe_id(id_sys):
                    print(f"  [DUP] ID duplicado detectado: {id_sys}")
                    id_dup      = f"{id_sys}_DUP_{datetime.now().strftime('%H%M%S')}"
                    ...
                    m.UnRead = False
                    cambios += 1
                    print(f"  ⚠️  Guardado: {id_dup}")
                    print(f"      Completa los campos manualmente en el Excel.")
                    continue
