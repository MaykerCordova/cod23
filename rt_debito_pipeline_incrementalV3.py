...
class RTDebitoTransformer:
    ...
    def parsear_fecha(df: pl.DataFrame) -> pl.DataFrame:
        """
        Parsea fechas para RT Débito.

        Bootstrap (Access): usa ACF_Fecha_TRX que viene como "01/11/2024" (DD/MM/YYYY)
        Daily (Excel): usa ACF-FECHA TRX que viene como "2024/11/01" (YYYY/MM/DD) o "20250625" (YYYYMMDD)

        Genera: ACF_Fecha_TRX (Date), Anomes_TRX, Dia_TRX
        """
        # Caso 1: Bootstrap - ACF_Fecha_TRX ya existe y viene del Access (DD/MM/YYYY)
        if "ACF_Fecha_TRX" in df.columns and df["ACF_Fecha_TRX"].dtype == pl.Utf8:
            df = df.with_columns(
                pl.col("ACF_Fecha_TRX")
                .str.strip_chars()
                .str.to_date("%d/%m/%Y", strict=False)
                .alias("ACF_Fecha_TRX")
            )
        # Caso 2: Daily - parsear desde ACF-FECHA TRX
        elif "ACF-FECHA TRX" in df.columns:
...
class SilverLoader:
    ...
    def append_incremental(df_new: pl.DataFrame) -> dict:
        """Append directo al Silver existente. Sin deduplicación."""
        print("\n--- APPEND INCREMENTAL ---")
        ...
        print(f"  Registros agregados: {df_new.shape[0]:,}")
