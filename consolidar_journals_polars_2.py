...
def cargar_journal(ruta: str, etiqueta: str) -> pl.DataFrame:
    ...
    # Engine xlsx2csv: convierte el Excel a CSV en memoria y lo parsea con Polars.
    # Es más compatible que fastexcel para forzar todo como String.
    # skip_rows = HEADER_ROW salta las filas ANTES del header (fila 5 → skip 4)
    df = pl.read_excel(
        path,
        engine="xlsx2csv",
        xlsx2csv_options={"skip_empty_lines": False},
        read_csv_options={
            "skip_rows": HEADER_ROW,        # salta las 4 filas previas al header
            "infer_schema_length": 0,       # todo como String (equiv. dtype=str)
            "truncate_ragged_lines": True,  # tolera filas con columnas de más
        },
    )
