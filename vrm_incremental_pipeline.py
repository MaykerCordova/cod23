...
class Config:
    ...
    # === REGLAS DE NEGOCIO ===
    BIN_CCF = "422052"  # BIN para entidad CCF (Santander)
...
class VRMExtractor:
    ...
    def leer_listas(lista1_path: str, lista2_path: str, encoding: str = "utf-8") -> pl.DataFrame:
        ...
        """
        try:
            df1 = pl.read_csv(lista1_path, encoding=encoding, infer_schema_length=0)
        except Exception:
            print(f"  Reintentando Lista 1 con encoding latin-1...")
            df1 = pl.read_csv(lista1_path, encoding="latin-1", infer_schema_length=0)

        try:
            df2 = pl.read_csv(lista2_path, encoding=encoding, infer_schema_length=0)
        except Exception:
            print(f"  Reintentando Lista 2 con encoding latin-1...")
            df2 = pl.read_csv(lista2_path, encoding="latin-1", infer_schema_length=0)
...
class VRMTransformer:
    ...
    def parsear_timestamp(df: pl.DataFrame) -> pl.DataFrame:
        """
        Parsea el timestamp de VRM que viene en formato texto con meses en español.
        Ejemplo input: "31 mar. 2026 14:30:00" o similar
        ...
        # Intentar parseo directo primero (formato ISO o datetime estándar)
        try:
            df = df.with_columns(
            ...
            ts_col = pl.col("_ts_clean")
            for mes_es, mes_num in Config.MESES_MAP.items():
                ts_col_str = f"{mes_es}."
                df = df.with_columns(
                    pl.col("_ts_clean")
                    .str.replace(ts_col_str, f"{mes_num}", literal=True)
                    .alias("_ts_clean")
                )
                # Sin punto también
                df = df.with_columns(
            ...
            # Extraer componentes con regex flexible
            # Formato esperado después de reemplazo: "31 03 2026 14:30:00" o "31-03-2026 14:30:00"
            df = df.with_columns([
                # Intentar extraer día, mes, año del string limpio
                pl.col("_ts_clean")
                .str.extract(r"(\d{1,2})[\s\-/]+(\d{1,2})[\s\-/]+(\d{4})", 1)
                .cast(pl.Int32, strict=False)
                .alias("_dia"),

                pl.col("_ts_clean")
                .str.extract(r"(\d{1,2})[\s\-/]+(\d{1,2})[\s\-/]+(\d{4})", 2)
                .cast(pl.Int32, strict=False)
                .alias("_mes"),

                pl.col("_ts_clean")
                .str.extract(r"(\d{1,2})[\s\-/]+(\d{1,2})[\s\-/]+(\d{4})", 3)
                .cast(pl.Int32, strict=False)
                .alias("_anio"),
            ])
            ...
            print("  IMPORTANTE: Revisa el formato del timestamp en tus CSVs.")
            print("  Puedes imprimir las primeras filas de '_TIMESTAMP_RAW' para ver el formato.")
            raise
    ...
    def aplicar_reglas_negocio(df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns([
            # Entidad
            pl.when(pl.col("BIN").cast(pl.Utf8).str.strip_chars() == Config.BIN_CCF)
            .then(pl.lit("CCF"))
