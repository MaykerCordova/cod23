...
for mes in meses_disponibles:
    sub_mes    = df[df["MES"] == mes]                          # todas (ap + den)
    sub_ap     = df_ap[df_ap["MES"] == mes]                   # solo aprobadas
    sub_f_tot  = sub_mes[sub_mes["ES_FRAUDE"] == 1]           # fraude total
    sub_f_ap   = sub_ap[sub_ap["ES_FRAUDE_APROBADO"] == 1]    # fraude aprobado
    ...
    # Fraude total (aprobadas + denegadas con F)
    n_f_tot_m    = len(sub_f_tot)
    monto_f_tot_m= sub_f_tot["MONTO"].sum()

    # Fraude aprobado (solo aprobadas con F)
...
n_fraudes_tot= df["ES_FRAUDE"].sum()               # fraude total
n_fraudes    = df["ES_FRAUDE_APROBADO"].sum()       # fraude aprobado
monto_fraude = df.loc[df["ES_FRAUDE_APROBADO"]==1,"MONTO"].sum()
monto_f_tot  = df.loc[df["ES_FRAUDE"]==1,"MONTO"].sum()
df_f         = df[df["ES_FRAUDE_APROBADO"]==1].copy()
...
def construir_tabla_deciles(df_base, n_deciles=10, label="D"):
    ...
    for decil in df_base["DECIL"].cat.categories:
        ...
        # Fraude aprobado (F + APROBADA)
        sub_f_ap  = sub[(sub["ESTADO"] == "APROBADA") & (sub["ES_FRAUDE_APROBADO"] == 1)]
        # Fraude total (F en cualquier estado)
...
tabla_vel = (
    ...
    .agg(
        ...
        Fraudes_Total        =("ES_FRAUDE",           "sum"),
        Fraudes_Aprobado     =("ES_FRAUDE_APROBADO",  "sum"),
    ...
    .assign(Pct_fraudes_total=lambda x: x["Fraudes_Total"]/x["Fraudes_Total"].sum()*100)
)
...
fig2.add_trace(go.Bar(x=bkts, y=tabla_vel["Fraudes_Aprobado"].tolist(), name="Fraude Aprobado",
    marker_color=C_FRAUDE,
    text=[f"{v:,}" for v in tabla_vel["Fraudes_Aprobado"]], textposition="outside"), row=1, col=1)
fig2.add_trace(go.Bar(x=bkts, y=tabla_vel["Fraudes_Total"].tolist(), name="Fraude Total",
    marker_color="#8B0000",
    text=[f"{v:,}" for v in tabla_vel["Fraudes_Total"]], textposition="outside"), row=1, col=1)
...
fraudes_en_088_tot    = (df["COND_088"] & (df["ES_FRAUDE"]==1)).sum()
fraudes_fuera_088_tot = (~df["COND_088"] & (df["ES_FRAUDE"]==1)).sum()
fraudes_en_088_ap     = (df["COND_088"] & (df["ES_FRAUDE_APROBADO"]==1)).sum()
fraudes_fuera_088_ap  = (~df["COND_088"] & (df["ES_FRAUDE_APROBADO"]==1)).sum()
total_fraudes_g       = df["ES_FRAUDE"].sum()
total_fraudes_ap_g    = df["ES_FRAUDE_APROBADO"].sum()

print(f"  — FRAUDE TOTAL (aprobado + denegado):")
print(f"    Dentro de 088:  {fraudes_en_088_tot:,}  ({fraudes_en_088_tot/total_fraudes_g*100:.1f}%)" if total_fraudes_g > 0 else "")
print(f"    Fuera de 088:   {fraudes_fuera_088_tot:,}  ({fraudes_fuera_088_tot/total_fraudes_g*100:.1f}%)" if total_fraudes_g > 0 else "")
print(f"  — FRAUDE APROBADO:")
print(f"    Dentro de 088:  {fraudes_en_088_ap:,}  ({fraudes_en_088_ap/total_fraudes_ap_g*100:.1f}%)" if total_fraudes_ap_g > 0 else "")
print(f"    Fuera de 088:   {fraudes_fuera_088_ap:,}  ({fraudes_fuera_088_ap/total_fraudes_ap_g*100:.1f}%)" if total_fraudes_ap_g > 0 else "")

comp_088 = pd.DataFrame({
    "Con 088": {
        "Transacciones"          : df["COND_088"].sum(),
        "Fraude Total"           : fraudes_en_088_tot,
        "Fraude Aprobado"        : fraudes_en_088_ap,
        "FR% (aprobado)"         : df.loc[df["COND_088"] & df["INDICADOR"].isin(["F","G"]),
                                         "ES_FRAUDE_APROBADO"].mean()*100
        ...
        "Monto fraude aprobado"  : df.loc[df["COND_088"] & (df["ES_FRAUDE_APROBADO"]==1),"MONTO"].sum(),
    },
    "Sin 088": {
        "Transacciones"          : (~df["COND_088"]).sum(),
        "Fraude Total"           : fraudes_fuera_088_tot,
        "Fraude Aprobado"        : fraudes_fuera_088_ap,
        "FR% (aprobado)"         : df.loc[~df["COND_088"] & df["INDICADOR"].isin(["F","G"]),
                                         "ES_FRAUDE_APROBADO"].mean()*100
                                   if (~df["COND_088"] & df["INDICADOR"].isin(["F","G"])).sum() > 0 else 0,
        "Monto prom (S/)"        : df.loc[~df["COND_088"],"MONTO"].mean(),
...
df_top_comercios = (
    ...
    .agg(
        ...
        Fraude_Total      =("ES_FRAUDE",            "sum"),
        Fraude_Aprobado   =("ES_FRAUDE_APROBADO",   "sum"),
        Monto_fraude_tot  =("MONTO",               lambda x: x[df_ap.loc[x.index,"ES_FRAUDE"]==1].sum()),
    ...
    .rename(columns={
        ...
        "Monto_fraude_tot": "Monto Fraude Total S/",
...
df_f_tot     = df[df["ES_FRAUDE"] == 1]           # fraude total
df_f_ap      = df[df["ES_FRAUDE_APROBADO"] == 1]  # fraude aprobado
...
stats_monto = pd.DataFrame({
    ...
    "Fraude Total (F)": (
        [df_f_tot["MONTO"].mean(), df_f_tot["MONTO"].median(),
         df_f_tot["MONTO"].std(),  df_f_tot["MONTO"].var(),
         df_f_tot["MONTO"].min(),  df_f_tot["MONTO"].max()] +
        [df_f_tot["MONTO"].quantile(p/100) for p in pcts]
    ) if len(df_f_tot) > 0 else [0]*13,
    "Fraude Aprobado": (
        [df_f_ap["MONTO"].mean(), df_f_ap["MONTO"].median(),
         df_f_ap["MONTO"].std(),  df_f_ap["MONTO"].var(),
         df_f_ap["MONTO"].min(),  df_f_ap["MONTO"].max()] +
        [df_f_ap["MONTO"].quantile(p/100) for p in pcts]
    ) if len(df_f_ap) > 0 else [0]*13,
    "No Fraude": (
...
for col in ["Total aprobadas","Fraude Total (F)","Fraude Aprobado","No Fraude"]:
...
with pd.ExcelWriter(RUTA_EXCEL_OUT, engine="openpyxl") as writer:
    ...
    agregar_interpretacion(ws, fila_interp+1, n_cols_res,
        f"Fraude total (F): {n_fraudes_tot:,} trxs — "
        f"Fraude aprobado: {n_fraudes:,} trxs ({n_fraudes/n_fraudes_tot*100:.1f}% del total). "
        f"La diferencia corresponde a fraudes en transacciones denegadas."
        if n_fraudes_tot > 0 else "Sin fraudes en el período.")
    ajustar_columnas(ws)
    ...
    agregar_interpretacion(ws, fi, len(stats_monto.columns),
        f"Mediana fraude aprobado S/ {med_f_ap:,.2f} vs fraude total S/ {med_f_tot:,.2f} "
        f"vs legítimo S/ {med_nof:,.2f}.")
