...
class VRMExtractor:
    ...
    @staticmethod
    def leer_listas(lista1_path: str, lista2_path: str, encoding: str = "latin-1") -> pl.DataFrame:
        """
        ...
        Args:
            lista1_path: Ruta al CSV de la Lista 1
            lista2_path: Ruta al CSV de la Lista 2
            encoding: Encoding del archivo (default: latin-1 para VRM)
        ...
        df1 = pl.read_csv(lista1_path, **read_opts)
        df2 = pl.read_csv(lista2_path, **read_opts)
...
class VRMTransformer:
    ...
    def aplicar_reglas_negocio(df: pl.DataFrame) -> pl.DataFrame:
        ...
        - TARJETA: limpiar caracteres ="..." que vienen del CSV
        - Entidad: CSF si BIN == 422052, sino SBP
        - Gestion: GESTIONADA si ID ANALISTA tiene valor real, sino NO GESTIONADA
        - Cuenta: siempre "1"
        - Fuente: siempre "VRM"
        - Limpieza de COD REDP 59
        """
        df = df.with_columns([
            # Limpieza de TARJETA: quitar ="..." que viene del CSV
            pl.col("TARJETA")
            .cast(pl.Utf8)
            .str.replace_all(r'[=""]', "")
            .str.strip_chars()
            ...
            # Limpieza de VISA TRANSSACTION ID: mismo problema de ="..."
            pl.col("VISA TRANSSACTION ID")
            .cast(pl.Utf8)
            .str.replace_all(r'[=""]', "")
            .str.strip_chars()
            .alias("VISA TRANSSACTION ID"),

            # Entidad: BIN 422052 = CSF, resto = SBP
            pl.when(pl.col("BIN").cast(pl.Utf8).str.strip_chars() == Config.BIN_CSF)
            .then(pl.lit("CSF"))
            .otherwise(pl.lit("SBP"))
            .alias("Entidad"),

            # Gestion: GESTIONADA si ID ANALISTA tiene valor real (no null, no vacío, no "N.A.")
            pl.when(
            ...
            # Limpieza de NUMERO DE TOKEN: quitar ="..."
            pl.col("NUMERO DE TOKEN")
            .cast(pl.Utf8)
            .str.replace_all(r'[=""]', "")
            .str.strip_chars()
            .alias("NUMERO DE TOKEN"),
        ])
